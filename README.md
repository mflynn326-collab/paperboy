...
4. **Index everything.** `index-corpus` builds a full-text search index over abstracts and extracted PDF text in ~2,800-character chunks. When Claude answers a question it retrieves only the few relevant passages — never whole papers — so responses stay fast and grounded. Pass `--include-pdfs` to add extracted PDF text on an incremental run. `--rebuild` clears and rebuilds the whole index; if the index already contains PDF chunks, a rebuild automatically re-extracts and re-indexes the PDFs so the full-text layer is never silently lost (add `--skip-pdfs` if you really want an abstracts-only rebuild — it prints a warning and drops the PDF chunks).
